...
void serve_directory(int fd, char *path, char *base_path) {
    ...
    while ((dir = readdir(d)) != NULL) {
        ...
        if (base_path[strlen(base_path) - 1] != '/') http_send_string(fd, "/");
 ...
 * port=server_proxy_port) and relays traffic to/from the stream client_fd and the
 * proxy target. HTTP requests from the client (client_fd) should be sent to the
 * proxy target, and HTTP responses from the proxy target should be sent to
 * the client (client_fd).
 *
 *   +--------+     +------------+     +--------------+
...
_Noreturn void thread_func(void (*request_handler)(int)) {
    while (1) {
